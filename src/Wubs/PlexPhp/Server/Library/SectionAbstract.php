<?php
namespace Wubs\PlexPhp\Server\Library;

use Wubs\PlexPhp\Exception\Library\LibraryException;
use Wubs\PlexPhp\Server\Library;
use Wubs\PlexPhp\Server\Library\Section\Show;
use Wubs\PlexPhp\Server\Library\ItemAbstract\ItemAbstract;
...
abstract class SectionAbstract extends Library
{
	...
	protected function getPolymorphicItem($polymorphicData, $scopedToItem = FALSE)
	{
		...
		} else if (strpos($polymorphicData, Library::ENDPOINT_METADATA)
			!== FALSE) {
...
//		$class = ucfirst($type);
		$class = sprintf(
			'Wubs\\PlexPhp\\Server\\Library\\Section\\%s',
			ucfirst($type)
		);
